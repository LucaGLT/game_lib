#ifndef GMTRIS_TRISRULECONTEXT_HPP
#define GMTRIS_TRISRULECONTEXT_HPP

/**
 * @file rules/TrisRuleContext.hpp
 * @brief gmRules::RuleContext adapter exposing the Tic-Tac-Toe board.
 *
 * The adapter models each board cell as a gmRules location whose id is the
 * 1-based cell index ("1".."9") matching Board::location_of(). The mark placed
 * in a cell is exposed as the location tag "mark_X" / "mark_O", so the win
 * condition can be expressed with LOCATION_HAS_TAG conditions.
 *
 * Only the location queries are meaningful for Tic-Tac-Toe; every other
 * RuleContext capability (actors, decks, dice, mutations) is a no-op or returns
 * a neutral default, since win detection is side-effect free.
 */
...
/**
 * @class TrisRuleContext
 * @brief Read-only RuleContext backed by a Tic-Tac-Toe Board.
 */
class TrisRuleContext : public gmRules::RuleContext
{
  public:
	/// @brief Builds an adapter over @p board (must outlive the context).
	explicit TrisRuleContext(const Board &board);

	/// @brief Returns the location id ("1".."9") for a 1-based cell.
	static std::string cell_id(uint8_t row, uint8_t col);

	/// @brief Returns the location tag carried by a placed @p mark.
	static std::string mark_tag(Mark mark);

	// ── Location queries (the only meaningful capabilities) ───────────────────
	...
	void modify_resource(const gmRules::ActorId &actor_id,
	                     const std::string &resource_id, int delta) override;
	void set_resource_max(const gmRules::ActorId &actor_id,
	                      const std::string &resource_id, int max_value) override;
	...
	bool card_in_zone(const gmRules::DeckId &deck_id, const gmRules::CardId &card_id,
	                  const std::string &zone_name) const override;
...
#endif // GMTRIS_TRISRULECONTEXT_HPP
