 ...
 * @brief Implementation of inbound command server for Dungeon GUI.
 */
...
static void close_native_socket(SocketHandle s)
{
	::closesocket(s);
}
...
bool recv_exact(SocketHandle sock, char* buffer, std::size_t n)
{
	...
	while (received < n)
	{
		int chunk = ::recv(sock, buffer + received, static_cast<int>(n - received), 0);
		if (chunk <= 0)
		{
			return false;
		}
	...
	while (_running)
	{
		...
		while (_running && recv_frame(client, raw))
		{
			try
			{
				...
				const std::string type_id = msg.value("typeId", "");
