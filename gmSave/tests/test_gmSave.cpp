 ...
 * Use @ref check, @ref check_throws, and @ref check_no_throw to register results.
 */
struct TestRunner
{
	gmLog::GmLogger& log;
	...
	void check(bool condition, const std::string& test_name)
	{
		...
		{
			++passed;
			logInfo(log, "[PASS] " + test_name);
		}
		else
		{
			++failed;
			logErr(log, "[FAIL] " + test_name);
		}
	}
	...
	void check_throws(const std::string& test_name, Fn&& fn)
	{
		try
		{
			fn();
			++failed;
			logErr(log, "[FAIL] " + test_name + " — expected exception not thrown");
		}
		catch (const ExcType&)
		{
			++passed;
	...
	void check_no_throw(const std::string& test_name, Fn&& fn)
	{
		try
		{
			fn();
...
static void test_versioned_roundtrip(TestRunner& tr)
{
	...
	tr.check_no_throw("T08 save_versioned completes without exception", [&]
	{
...
// T09 — load_versioned throws EVersionMismatchError on wrong version
static void test_versioned_mismatch(TestRunner& tr)
{
	...
	tr.check_throws<gmSave::EVersionMismatchError>(
		"T09 load_versioned throws EVersionMismatchError on wrong version",
		[&]
		{
...
// T09b — EVersionMismatchError carries the correct version numbers
static void test_versioned_mismatch_fields(TestRunner& tr)
{
	...
	try
	{
		...
		tr.check(false, "T09b EVersionMismatchError fields — exception not thrown");
	}
	catch (const gmSave::EVersionMismatchError& e)
	{
		tr.check(e.expected_version == 5 && e.found_version == 1,
				 "T09b EVersionMismatchError carries correct expected/found versions");
	}
	catch (...)
	{
		tr.check(false, "T09b EVersionMismatchError fields — wrong exception type");
	}
}

// T10 — load_versioned throws EJsonParseError when _version field is missing
static void test_versioned_missing_version_field(TestRunner& tr)
{
	...
	tr.check_throws<gmSave::EJsonParseError>(
		"T10 load_versioned throws EJsonParseError on missing _version field",
		[&]
		{
...
// T13 — EFileReadError thrown on non-existent file
static void test_fileread_error(TestRunner& tr)
{
	tr.check_throws<gmSave::EFileReadError>(
		"T13 load throws EFileReadError on non-existent file",
		[]
		{
...
// T14 — EFileWriteError thrown on unwritable path
static void test_filewrite_error(TestRunner& tr)
{
	// A path whose parent directory does not exist will reliably fail.
	tr.check_throws<gmSave::EFileWriteError>(
		"T14 save throws EFileWriteError on unwritable path",
		[]{
...
// T15 — EJsonParseError thrown on invalid JSON content
static void test_jsonparse_error(TestRunner& tr)
{
	...
	tr.check_throws<gmSave::EJsonParseError>(
		"T15 load throws EJsonParseError on invalid JSON content",
		[&]
		{
