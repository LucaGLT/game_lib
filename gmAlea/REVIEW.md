...
> namespace `gmAlea` → `gmAlea`, façades `GmDeck`/`GmCompDeck`) with a
...
#### F-01 · NS-1 🔴 Error — namespace `gmAlea` must become `gmAlea`

**Files:** `GmDeck.hpp`, `GmDeck.cpp`, `GmCompDeck.hpp`, `GmCompDeck.cpp`,
`PolicyBasedDeck.hpp`, `ZonePolicy.hpp`, `CardLocation.hpp`  
**Location:** every `namespace gmAlea {` / `} // namespace gmAlea`  
**Description:** The project namespace is `gm` + PascalCase.  The current
name `gmAlea` is being replaced by `gmAlea` as part of the library rename.
All seven files declare or close `namespace gmAlea`.
...
**File:** `GmDeck.hpp`  
**Location:** `class EAleaError`, line ~13  
...
`EAleaError` violates both the prefix rule (EX-1) and the base-naming
rule (EX-2).
...
#### F-05 · EX-1 🔴 Error — `EAleaDeckEmptyError` missing `E` prefix

**File:** `GmDeck.hpp`  
**Location:** `class EAleaDeckEmptyError`  
...
#### F-06 · EX-1 🔴 Error — `EAleaDuplicateTokenIdError`, `EAleaInvalidDrawCountError`, `EAleaTokenNotFoundError` missing `E` prefix

**File:** `GmDeck.hpp`  
**Location:** `class EAleaDuplicateTokenIdError`, `class EAleaInvalidDrawCountError`,
`class EAleaTokenNotFoundError`  
...
#### F-07 · EX-1 🔴 Error — `EAleaZonePolicyViolationError` missing `E` prefix and `Error` suffix

**File:** `PolicyBasedDeck.hpp`  
**Location:** `class EAleaZonePolicyViolationError`  
**Description:** Exception class missing both the `E` prefix and the required
`Error` suffix on the name.

```diff
- class EAleaZonePolicyViolationError : public std::runtime_error {
...
#### F-09 · IG-1 🔴 Error — all guards use `GMALEA_` prefix instead of `GMALEA_`
...
| Current | Corrected |
|---------|-----------|
| `GMALEA_GMDECK_HPP` | `GMALEA_GMDECK_HPP` |
| `GMALEA_GMCOMPDECK_HPP` | `GMALEA_GMCOMPDECK_HPP` |
| `GMALEA_POLICYBASEDDECK_HPP` | `GMALEA_POLICYBASEDDECK_HPP` |
| `GMALEA_ZONEPOLICY_HPP` | `GMALEA_ZONEPOLICY_HPP` |
| `GMALEA_CARDLOCATION_HPP` | `GMALEA_CARDLOCATION_HPP` |
...
**Description:** The usage example references `gmAlea::GmCompDeck` which
