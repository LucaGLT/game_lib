#ifndef GMALEA_POLICYBASEDDECK_HPP
#define GMALEA_POLICYBASEDDECK_HPP
...
{
public:
	explicit EAleaZonePolicyViolationError(const std::string& message)
		: std::runtime_error("EAleaZonePolicyViolationError: " + message) {}
};
 ...
 * ### Example
 * @code
 *   gmAlea::DiscardPile discard("DISCARD");
 *   discard.add(42);
 *   discard.add(17);
...
public:
	 ...
	 * @throws EAleaDuplicateTokenIdError if `initial_tokens` contains duplicates.
	 */
	PolicyBasedDeck(std::string zone_name,
					const std::vector<uint32_t>& initial_tokens,
					std::optional<unsigned int> seed = std::nullopt)
		: _zone_name(std::move(zone_name))
		, _deck(initial_tokens, seed, Policy::can_shuffle)
	{}
	 ...
	 * @throws EAleaDuplicateTokenIdError if token_id is already in this zone.
	 */
	 ...
	 * @throws EAleaDeckEmptyError if the zone is empty.
	 *
	 ...
	 * @throws EAleaTokenNotFoundError if token_id is not in this zone.
	 *
	...
	uint32_t take_specific(uint32_t token_id)
	{
		static_assert(
			...
			"(e.g. BanishZone).");
		return _deck.draw_specific(token_id);
	}
	...
	{
		std::vector<uint32_t> all = _deck.peek_all();
...
#endif // GMALEA_POLICYBASEDDECK_HPP
