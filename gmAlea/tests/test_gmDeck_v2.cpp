 ...
 *        auto_shuffle flag, and the new EAleaTokenNotFoundError exception.
 *
...
static void test_push_back_duplicate()
{
	...
	} catch (const EAleaDuplicateTokenIdError&)
	{
		pass(T);
	} catch (...)
	{
...
// ── Test 8: draw_specific on missing token throws EAleaTokenNotFoundError ──────────
...
static void test_draw_specific_not_found()
{
	...
	} catch (const EAleaTokenNotFoundError&)
	{
		pass(T);
	} catch (...)
	{
