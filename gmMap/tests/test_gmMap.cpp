...
bool contains_tile_id(const std::vector<gmMap::TileId>& ids, gmMap::TileId id)
{
	return std::find(ids.begin(), ids.end(), id) != ids.end();
}
...
struct TestRunner
{
	...
	explicit TestRunner(const std::string& logPath)
	    : logger(gmLog::LoggerFactory::create_file_logger(
	          "gmMapPhase11", logPath, gmLog::LogLevel::DEBUG, true)),
	      passed(0), failed(0)
	{
	}
	...
	int summary()
	{
		...
		const std::string msg = "=== gmMap Phase 11 summary: " + std::to_string(passed) + "/" +
...
bool test_tile_crud()
{
	...
	std::vector<gmMap::TileId> all = m.all_tiles();
	if (all.size() != 2) return false;

	m.remove_tile(10);
	if (m.has_tile(10)) return false;
	if (m.tile_count() != 1) return false;
	if (!contains_tile_id(m.all_tiles(), 11)) return false;

	return true;
}

bool test_assignment_and_reassignment()
{
	TestMap m;
	...
	std::optional<gmMap::TileId> t1 = m.tile_of(100);
	if (!t1.has_value() || t1.value() != 1) return false;

	m.assign_to_tile(100, 2); // re-assignment
	std::optional<gmMap::TileId> t2 = m.tile_of(100);
	if (!t2.has_value() || t2.value() != 2) return false;

	std::vector<gmMap::LocationId> in1 = m.locations_in_tile(1);
	std::vector<gmMap::LocationId> in2 = m.locations_in_tile(2);

	if (contains_id(in1, 100)) return false;
	if (!contains_id(in2, 100)) return false;

	m.unassign_from_tile(100);
	std::optional<gmMap::TileId> t3 = m.tile_of(100);
...
bool test_tile_metadata()
{
	...
	m.set_tile_meta(99, "occupants", gmMap::UidList{gmMap::UidRef{77U}, gmMap::UidRef{88U}});
	...
	const gmMap::MetadataValue& z = m.get_tile_meta(99, "zone");
	const gmMap::MetadataValue& l = m.get_tile_meta(99, "level");
	const gmMap::MetadataValue& o = m.get_tile_meta(99, "occupants");
	...
	const gmMap::Metadata& meta = m.tile_metadata(99);
	if (meta.size() != 3) return false;
...
bool test_remove_location_cascade()
{
	...
	std::vector<gmMap::LocationId> inTile = m.locations_in_tile(9);
	if (contains_id(inTile, 1)) return false;
	if (!contains_id(inTile, 2)) return false;
...
bool test_remove_tile_cascade()
{
	...
	std::optional<gmMap::TileId> t10 = m.tile_of(10);
	std::optional<gmMap::TileId> t11 = m.tile_of(11);
...
int main()
{
	...
	tr.check("T04 tile CRUD + invariants", test_tile_crud());
	tr.check_throws<gmMap::EDuplicateTileError>("T05 duplicate tile create throws",
	                                            []()
	                                            {
		                                            TestMap m;
		                                            m.create_tile(8);
	...
	tr.check_throws<gmMap::EUnknownTileError>("T06 remove missing tile throws",
	                                          []()
	                                          {
		                                          TestMap m;
		                                          m.remove_tile(808);
	...
	tr.check("T07 assignment + reassignment", test_assignment_and_reassignment());
	tr.check_throws<gmMap::EUnknownLocationError>("T08 assign unknown location throws",
	                                              []()
	                                              {
		                                              TestMap m;
		                                              m.create_tile(1);
	...
	tr.check_throws<gmMap::EUnknownTileError>("T09 assign unknown tile throws",
	                                          []()
	                                          {
		                                          TestMap m;
		                                          m.create_location(1);
	...
	tr.check("T10 adjacency directed/bidirectional", test_adjacency_directed_bidirectional());
	tr.check_throws<gmMap::EInvalidAdjacencyError>("T11 self-loop rejected",
	                                               []()
	                                               {
		                                               TestMap m;
		                                               m.create_location(1);
	...
	tr.check("T12 items add/remove/out-of-range base", test_items());
	tr.check_throws<gmMap::EInvalidItemIndexError>("T13 remove_item out-of-range throws",
	                                               []()
	                                               {
		                                               TestMap m;
		                                               m.create_location(1);
	...
	tr.check("T14 location metadata", test_location_metadata());
	tr.check_throws<gmMap::EUnknownMetaKeyError>("T15 missing location metadata key throws",
	                                             []()
	                                             {
		                                             TestMap m;
		                                             m.create_location(1);
	...
	tr.check("T16 tile metadata", test_tile_metadata());
	tr.check_throws<gmMap::EUnknownMetaKeyError>("T17 missing tile metadata key throws",
	                                             []()
	                                             {
		                                             TestMap m;
		                                             m.create_tile(1);
	...
	tr.check("T18 clear full reset", test_clear_full_reset());
	tr.check("T19 remove_location cascade", test_remove_location_cascade());
	tr.check("T20 remove_tile cascade", test_remove_tile_cascade());
