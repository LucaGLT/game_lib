...
struct TestRunner
{
	...
	explicit TestRunner(const std::string& logFile)
	    : logger(gmLog::LoggerFactory::create_file_logger(
	          "gmMapTest", logFile, gmLog::LogLevel::DEBUG, true))
	{
	}
	...
	void check_throws(const std::string& testName, const std::function<void()>& fn)
	{
		++total;
		try
		{
			...
			logger.log(gmLog::LogLevel::ERROR,
			           "[FAIL] " + testName + " (expected exception, none thrown)");
		}
		catch (const std::exception& e)
		{
			++passed;
			logger.log(gmLog::LogLevel::INFO,
			           "[PASS] " + testName + " (exception: " + std::string(e.what()) + ")");
		}
	}

	void summary()
	{
		std::string msg = "=== gmMap Phases 2-4 test summary: " + std::to_string(passed) + "/" +
...
int main()
{
	try
	{
		...
		using SimpleMap = ::gmMap::gmMap<int>;
		...
		runner.check("T02: clear() removes all locations",
		             [&]()
		             {
			             ...
			             m.create_location(3);
			             m.clear();
			             return m.location_count() == 0 && !m.has_location(1) &&
			                    !m.has_location(2) && !m.has_location(3);
		             }());
		...
		runner.check_throws("T04: create_location throws on duplicate",
		                    [&]()
		                    {
			                    SimpleMap m;
			                    m.create_location(10);
		...
		runner.check_throws("T10: remove_location throws on missing",
		                    [&]()
		                    {
			                    SimpleMap m;
		...
		runner.check("T12: Complex CRUD scenario",
		             [&]()
		             {
			             ...
			             if (!m.has_location(11) || m.has_location(12) || !m.has_location(13))
				             return false;
			             m.clear();
			             return m.location_count() == 0;
		             }());
