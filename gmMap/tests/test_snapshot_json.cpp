...
bool testSnapshotRoundTrip()
{
    ...
    m1.set_location_meta(1, "owner_id", GameMap::UidRef{1001U});
    m1.set_location_meta(2, "name", std::string("Location B"));
    m1.set_location_meta(3, "occupants", GameMap::UidList{{1001U}, {1002U}});
    ...
    // Verify locations
    std::vector<LocationId> locs1 = m1.all_locations();
    std::vector<LocationId> locs2 = m2.all_locations();
    if (locs1.size() != locs2.size() || locs1.size() != 3) {
        std::cerr << "ERROR: Location count mismatch: " << locs1.size() << " vs " << locs2.size() << std::endl;
        return false;
    }

    // Verify tiles
    std::vector<GameMap::TileId> tiles1 = m1.all_tiles();
    std::vector<GameMap::TileId> tiles2 = m2.all_tiles();
    ...
    // Verify assignments
    for (GameMap::LocationId loc : {1, 2, 3}) {
    ...
    if (items1_loc1.size() != 2 || items2_loc1.size() != 2 ||
        items1_loc1[0] != 10 || items2_loc1[0] != 10 ||
        items1_loc1[1] != 20 || items2_loc1[1] != 20) {
    ...
    try {
        ...
        if (std::get<GameMap::UidRef>(uid1).value != std::get<GameMap::UidRef>(uid2).value) {
        ...
        const auto& list1 = std::get<GameMap::UidList>(occupants1);
        const auto& list2 = std::get<GameMap::UidList>(occupants2);
        if (list1.size() != 2 || list2.size() != 2 ||
            list1[0].value != list2[0].value || list1[1].value != list2[1].value) {
...
int main()
{
    ...
    pass = testSnapshotRoundTrip() && pass;
    std::cout << (pass ? "✓" : "✗") << " testSnapshotRoundTrip\n";
