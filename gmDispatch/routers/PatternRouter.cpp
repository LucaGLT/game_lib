...
bool PatternRouter::matchPattern(const std::string& pattern,
...
bool PatternRouter::isTargeted(const std::shared_ptr<IChannel>& channel,
...
{
    routes_[pattern].push_back(std::move(channel));
}
...
void PatternRouter::route(const Envelope& envelope)
{
    for (std::pair<const std::string,
        ...
        if (!matchPattern(kv.first, envelope.typeId)) continue;

        for (std::shared_ptr<IChannel>& ch : kv.second) {
            if (isTargeted(ch, envelope)) {
                ch->send(envelope);
            }
        }
    }
