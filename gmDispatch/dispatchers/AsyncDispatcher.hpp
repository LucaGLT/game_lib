 ...
 * | Mutex         | Protects                                         |
 * |---------------|--------------------------------------------------|
 * | `queueMutex_` | `queue_`, `workerBusy_`, `stop_`, `queueCv_`, `drainCv_` |
 * | `routeMutex_` | `router_` (subscribe / unsubscribe / route / flush) |
 *
 * ### Deadlock warning
 * Do not call `subscribe()`, `unsubscribe()`, `dispatch()`, or `flush()` on
 * the **same** `Dispatcher` from within a channel's `send()` callback.
 * Doing so will deadlock on `routeMutex_`.
 *
 * @par Example
 * @code
...
private:
    /// Worker thread body — drains the queue and calls router_->route().
    void workerLoop();
    ...
    std::condition_variable queueCv_;   ///< Worker wakes up when queue is non-empty.
    std::condition_variable drainCv_;   ///< flush() wakes up when queue is empty.
