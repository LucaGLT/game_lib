...
# GUI Style Instructions — {{PROJECT_NAME}} / GUI Module

> These instructions apply automatically to every GUI source file.
> The authoritative source specification is `.github/specs/gui-theme.yml`.
> Target toolkit: `{{GUI_TOOLKIT}}`.
...
All visual decisions must derive from the **active theme** via QSS (or the toolkit's equivalent
...
| GUI-4 | No hardcoded border style strings in widget code. |
...
| GUI-10 | Every widget renders correctly under all defined themes (`{{THEME_IDS}}`). |
...
Never implement state changes via hardcoded color assignments in code (Rule WS-1).
...
- No inline method bodies that perform visual configuration via hardcoded properties.
...
- Widgets must render correctly under all defined themes without any code change:
  `{{THEME_IDS}}`.
...
Domain or business-logic values must never drive a visual output directly (Rule GUI-9).
...
# Correct — state exposed via a dynamic property; QSS reacts
...
Blur radius must not exceed `8 px`.
