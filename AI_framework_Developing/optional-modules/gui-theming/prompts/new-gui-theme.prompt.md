---
mode: agent
description: "Define a new visual theme compliant with the GUI theme system."
---

# New GUI Theme

Define a new visual theme for the `{{PROJECT_NAME}}` GUI module.
...
1. **Theme id** — lowercase `snake_case` identifier (e.g. `forest`, `ocean_depths`). Must be
   added to the project's `{{THEME_IDS}}` list.
...
- Domain or business-logic information must not appear in color variable names or comments.
- The new theme must produce correct rendering for all widgets without any
  modification to widget source code.
