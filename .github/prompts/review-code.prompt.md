---
mode: agent
description: "Analyse existing C++ code for style/naming violations and produce a structured REVIEW.md correction plan."
---

# Review Existing Code

Analyse one or more existing C++ files (or an entire library) for compliance with
game_lib style and coding rules. Produce a structured `REVIEW.md` correction plan.
...
Use `.github/specs/cpp-style.yml` as the authoritative rule reference.
...
1. **Scope** — choose one:
   ...
   - `library` — review all `.hpp` and `.cpp` files inside a given library folder.
   - `workspace` — review all C++ files in the workspace.
2. **Paths** — file paths or library name (if scope is `file` or `library`).
3. **Categories** — which rule categories to check (default: all).
   Categories: `naming`, `guards`, `formatting`, `spacing`, `switch`,
...
4. **Output location** — where to write `REVIEW.md` (default: inside the reviewed
   library folder, or workspace root for a workspace-wide review).
...
1. **Header block** — library or scope reviewed, date, reviewer (AI), rule set version.
